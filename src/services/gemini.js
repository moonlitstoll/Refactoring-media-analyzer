...
2. **전수 및 순차 분석 (누락 금지)**: 문장의 첫 단어부터 마지막 기능어(cho, đã, đang, mới, nhé 등)까지 단 하나도 빠짐없이 등장 순서대로 분석해. 건너뛰는 단어가 있으면 안 돼.
3. **중복 설명 허용**: 이전에 나온 단어라도 문장에 다시 등장하면 생략 없이 똑같이 다시 설명해.
4. **단어별 단일 행(One Line) 분석**: 모든 단어(복합어 포함) 설명은 반드시 **한 줄**에 끝내십시오. 다음절 단어나 복합어 설명 시, 음절별 의미는 별도의 행을 만들지 말고 해당 행 안에서 '+' 또는 '()'를 사용하여 한꺼번에 설명하십시오. (예: phó thủ lãnh đạo: 부지도자, 부총리 (phó (버금 부) + thủ: 머리, 수장 (머리 수) + lãnh đạo: 지도자 (lãnh: 거느릴 령 + đạo: 이끌 도)))
...
7. **미니멀리즘 유지**: 문법 용어('대명사', '동사' 등)는 삭제하고, 핵심 의미 위주로 모바일 가독성을 위해 가장 짧고 명확한 단어로 설명하십시오.
