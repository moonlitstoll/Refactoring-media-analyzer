...
4. 다음절 단어/복합어 분절 분석: **Thế thì**, **lần sau** 같이 여러 음절로 구성된 단어는 반드시 각 음절의 의미를 풀어 설명해줘. (예: **Thế thì**: 그렇다면, 그러면 (**Thế**: 그렇게 + **thì**: ~라면)).
5. 한자 기반 복합어 풀이: 한자 기반 단어는 반드시 각 음절의 한글 뜻을 풀어서 설명해. (예: tiền (돈 전) + cọc (보증)). 단, 한자(漢字) 자체는 표기하지 말고 한글 음과 뜻만 적어.
6. 패턴 볼드 강조: 회화 핵심 패턴은 반드시 **볼드** 처리하고, 상관 접속사처럼 짝이 있는 경우 **(A와 연결)**이라고 명시해. (예: **đâu có ... 오픈 ... đâu**: (패턴) 전혀 ~하지 않다).
7. 미니멀리즘 유지: 문법 용어('대명사' 등)와 너무 뻔한 정보(호칭의 성별 지칭 대상 등)는 모두 삭제하고 핵심 의미 위주로 콤팩트하게 보여줘. **청크와 청크 사이에는 반드시 빈 줄을 하나 두어 구분해.**
...
- **예시**:
**Thế thì lần sau...** (그렇다면 다음번에는...)
**Thế thì**: (패턴) 그렇다면, 그러면 (**Thế**: 그렇게 + **thì**: ~라면)
**lần sau**: 다음번 (**lần**: 번, 차례 + **sau**: 다음, 뒤)
