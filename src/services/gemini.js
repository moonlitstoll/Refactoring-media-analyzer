...
4. **계층적 한 줄 분석(Hierarchical One-Line)**: "원어 청크: 한국어 전체 뜻 (단어별 상세 풀이)" 형식만 사용하십시오. 줄 시작에 "청크:", "[분석]" 등 어떠한 레이블도 붙이지 마십시오.
5. **한자어 노이즈 제거 (순수 한글 뜻만)**: 한자 병기(예: 評價)를 절대 하지 마십시오. 오직 순수 한국어 뜻만 간결하게 제공하십시오.
6. **문법 패턴 통합 필축**: 상관 접속사 패턴(예: 'nếu...thì', 'càng...càng', 'đâu có...đâu' 등)을 최우선적으로 묶어서 설명하며, 반드시 **굵게(Bold)** 표시하십시오.
7. **초단축 미니멀리즘 (핵심 키워드)**: 모든 설명에서 서술형(~을 나타냄, ~임)을 배제하십시오. 오직 핵심 뜻과 기능 키워드(예: 완료, 존칭, 미래)만 단어 위주로 짧게 표기하십시오. 
...
**[7대 분석 규칙]**
1. **의미 단위(Chunk-centric) 통합 분석 (최우선)**: 청크 단위로 묶어서 분석하십시오.
2. **청크 내 전수 분석 (중복 재분석 금지)**: 청크 내부 단어를 상세히 풀이하되, 분석된 단어를 독립된 행으로 중복 출력하지 마십시오.
3. **반복 설명 허용**: 이전 문장에 나온 단어라도 현재 문장에서 쓰였다면 다시 설명하십시오.
4. **계층적 한 줄 분석(Hierarchical One-Line)**: "원어 청크: 한국어 뜻 (상세 풀이)" 형식만 사용하십시오.
5. **한자어 노이즈 제거**: 오직 순수 한국어 뜻만 간결하게 제공하십시오.
6. **문법 패턴 통합 필축**: 상관 접속사 패턴을 최우선으로 분석하며, 반드시 **굵게(Bold)** 표시하십시오.
7. **초단축 미니멀리즘**: 핵심 뜻과 기능 키워드 위주로 짧게 표기하십시오.
