...
[분석 및 언어학적 규칙]
1. 초간결성 원칙 (Extreme Conciseness):
   - 문화적 배경, 문장 내 역할, 화자의 태도 등에 대한 **장황한 부연 설명을 절대 하지 마십시오.**
   - 모든 해설은 핵심 의미와 구성요소 분해만 포함합니다.
2. 구성요소 분해 필수 (Mandatory Breakdown):
   - 모든 청크(Chunk)에 대해 반드시 개별 단어의 뜻을 분해하여 설명하십시오.
   - 형식: 뜻 / 뜻. (단어1: 뜻 + 단어2: 뜻)
   - 예: Thôi -> 에이 / 그만. (Thôi: 에이, 됐어, 그만해)
   - 예: hai anh em -> 우리 두 사람. (hai: 둘 + anh em: 형제/가까운 사이)
3. 중복 설명 필수: 이전 문장에서 나온 단어라도 현재 문장에 포함되어 있다면 반드시 다시 설명하십시오.
4. 의미 중심의 유연한 청크: 2음절 이상의 단어나 구문은 의미 전달이 가장 잘 되는 단위(청크)로 묶어서 분석하십시오.
