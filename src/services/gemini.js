...
**[7대 분석 규칙]**
1. **의미 청크(Semantic Chunk) 통합 분석 (절대 원칙)**: 단어를 하나하나 따로 설명하지 마십시오. 반드시 의미가 형성되는 최소 단위인 '청크(Chunk)'를 한 행에 묶어서 분석하십시오. **이때, 청크 내의 각 단어는 반드시 슬래시('/')로 구분하여 표기하십시오.**
   - 예: **Mình / có / thêm**: 나는 추가로 가지고 있다 (나 + 가지다 + 추가로)
2. **청크 내 전수 분석 (개별 행 생성 금지)**: 하나의 청크로 묶인 내부의 모든 구성 단어는 해당 행의 괄호'()' 안에서 풀이하십시오. **이미 청크에 포함된 단어를 하단에 별도의 행으로 다시 꺼내어 중복 분석하는 것을 엄격히 금지합니다.**
...
5. **한자 캐릭터 병기 절대 금지**: 한자 기반 단어 분석 시에도 **실제 한자 캐릭터(예: 評價, 星)**를 절대로 병기하지 마십시오. 오직 한글로 된 뜻풀이(음과 뜻)만 포함하십시오.
   ...
   - 잘못된 예: 평가하다 (評價) <- 절대 금지
6. **핵심 패턴 정밀 강조 (Selective Bold)**: 청크 전체를 볼드 처리하지 마십시오. 문법적 구조나 회화 핵심 **패턴(Pattern)**을 구성하는 핵심 단어만 **볼드** 처리하십시오. 또한, 분석 내용 끝에 어떤 패턴인지 명시하십시오.
   - 예: **có** thể ăn hết được **đâu** (패턴: **có ... đâu** - 강한 부정)
   - 예: **đâu có** ... **đâu**, **nếu** như ... **mà**, **Nếu**... **thì** 등
...
[번역] 번역된 한국어 문장
[분석] 원어 / 청크 / 구분: 한국어 전체 뜻 (단어: 뜻 + 단어: 뜻)
...
**[7대 분석 규칙]**
1. **의미 청크 통합 분석 (절대 원칙)**: '의미 청크' 단위로 묶어서 수행하되, 청크 내 단어는 '/'로 구분하십시오.
2. **청크 내 전수 분석 (개별 행 금지)**: 청크 내부 단어를 해당 행 안에서 상세히 풀이하되, 중복 출력하지 마십시오.
...
6. **핵심 패턴 정밀 강조**: 문법 핵심 **패턴(Pattern)** 단어만 **볼드** 처리하고, 분석 시 패턴명과 의미를 명시하십시오.
