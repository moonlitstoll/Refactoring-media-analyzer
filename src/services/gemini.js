...
[분석 및 언어학적 규칙]
1. 의미 중심의 청크 구성: 각 문장을 의미 및 문법적 연관성이 있는 '청크(Chunk)' 단위로 분해하십시오.
2. 유연한 구성요소 분해 (의미 우선):
   - 다음절어(2음절 이상)나 청크는 기본적으로 개별 음절/단어로 나누어 설명하되, **가장 중요한 기준은 '의미 전달력'**입니다.
   - 음절을 나누는 것이 의미 이해에 방해가 된다면 나누지 말고 통째로 설명하십시오.
   - 예: 'đội mũ' -> '모자를 쓰다 (đội: 쓰다 + mũ: 모자)' (분해)
   - 예: 'cám ơn' -> '감사합니다' (의미적으로 통째로 이해하는 것이 좋다면 유지)
3. 문맥 및 문법적 연결성: 설명을 작성할 때 해당 단어가 문장 내에서 어떻게 쓰였는지 문법적/의미적 연결성을 고려하여 작성하십시오.
4. 고속 응답 규칙: 불필요한 미사여구를 생략하고 [청크: 번역 (구성요소 해설)] 형태로 정보를 압축하십시오.

[응답 형식]
[
  [번호, "전체 한국어 번역", [["의미 청크", "해설 (구성요소 및 문법적 연결성)"], ...]],
  ...
]`,
            `분석 대상 (번호와 원문):\n${inputContent}`
        ]);
