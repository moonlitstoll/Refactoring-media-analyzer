...
**[7대 분석 규칙]**
1. **청크 우선 분석**: 의미가 하나로 연결되는 덩어리(Chunk, 예: cho cẩn thận, sẽ giúp) 단위로 먼저 나누어 보여주어 가독성을 확보하십시오.
2. **전수 및 순차 분석 (철저한 누락 금지)**: 문장의 첫 단어부터 마지막 기능어(cho, đã, đang, mới, nhé 등)까지 단 하나도 빠짐없이 순서대로 분석하십시오. 세밀해진 청크 내의 모든 단어를 심층 분석하십시오. 단, 6번의 '문법 패턴'에 해당하는 단어들은 해당 패턴 행에서 한꺼번에 분석하되, 문장 전체 흐름에서 누락되는 단어가 없어야 합니다.
3. **중복 설명 허용**: 이전에 나온 단어라도 문장에 다시 등장하면 생략 없이 똑같이 다시 설명하십시오.
4. **단어별 단일 행(One Line) 분석**: 모든 분석(청크/패턴 포함) 설명은 반드시 **한 줄**에 끝내십시오. 내부 구성 요소 풀이는 해당 행 안에서 '+' 또는 '()'를 사용하여 한꺼번에 설명하십시오. (예: phó thủ lãnh đạo: 부지도자 (phó (버금 부) + thủ: 머리 (머리 수) + lãnh đạo: 지도자 (lãnh: 거령 + đạo: 도)))
5. **한자 기반 풀이 통합**: 한자 기반 단어 분석 시에도 별도의 행을 생성하지 마십시오. 한글 뜻풀이를 단어 설명 행 안에 포함하십시오. (예: tiền cọc: 보증금 (tiền (돈 전) + cọc (보증)))
6. **문법 패턴 통합 필축 (최우선)**: 'nếu...thì', 'càng...càng', 'đâu có...đâu', 'not only... but also' 처럼 짝을 이루는 패턴은 문장 중간에 다른 단어가 있더라도 반드시 한 줄에서 함께 통합하여 설명하십시오. 이 때 패턴 부분은 **볼드** 처리하십시오.
7. **미니멀리즘 유지**: 문법 용어('대명사', '동사' 등)는 삭제하고, 핵심 의미 위주로 모바일 가독성을 위해 불필요한 부연 설명은 생략하십시오.
