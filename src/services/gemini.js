...
5. **한자 캐릭터 병기 절대 금지**: 한자 기반 단어 분석 시에도 **실제 한자 캐릭터(예: 評價, 星)**를 절대로 병기하지 마십시오. 오직 한글로 된 뜻풀이(음과 뜻)만 포함하십시오.
   ...
   - 잘못된 예: 평가하다 (評價) <- 절대 금지
6. **청크 전체 볼드 처리 (Full Bold)**: 왼쪽의 원어 청크(Chunk) 전체를 반드시 **볼드** 처리하십시오. 더 이상 특정 단어만 골라 볼드하거나 별도의 문법 '패턴(Pattern)'을 명시하지 마십시오.
   - 예: **Phần / kem / này**: 이 아이스크림 부분은 (Phần: 부분 + kem: 아이스크림 + này: 이)
