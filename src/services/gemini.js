...
**[7대 분석 규칙]**
1. **의미 단위(Chunk-first) 분석**: 문장을 자연스러운 의미 덩어리(Chunk)로 최우선 분할하십시오.
2. **전수 및 순차 분석 (철저한 누락 금지)**: 문장 내의 모든 단어를 순차적으로 분석하십시오. 'cho', 'đã', 'là'와 같은 기능어도 절대로 누락하지 말고 반드시 포함하십시오.
3. **반복 설명 허용**: 이전 문장에서 나온 단어라도 현재 문장에서 쓰였다면 생략하지 말고 다시 설명하십시오.
4. **계층적 한 줄 분석(Hierarchical One-Line)**: 청크 전체 뜻을 먼저 쓰고, 그 뒤에 구성 단어들의 개별 의미를 괄호'()'와 '+'를 사용하여 같은 줄에 포함하십시오. 단어 분석을 위해 줄을 바꾸지 마십시오. (형식: 청크: 뜻 (단어1: 뜻 + 단어2: 뜻))
5. **한자어 병기**: 한자 기반 단어는 반드시 한자 및 한국어 독음을 병기하십시오. (예: chuẩn bị(準備, 준비))
6. **문법 패턴 통합 필축 (최우선)**: 'nếu...thì', 'càng...càng' 등 상관 접속사나 문법 패턴은 절대로 쪼개지 말고 하나의 항목으로 묶어서 가장 먼저 설명하십시오. 패턴은 반드시 **굵게(Bold)** 표시하십시오.
...
**[구현 예시]**
문장: chua lòm phát ra từ trong bếp.
[번역] 시큼한 냄새가 부엌 안에서 풍겨 나왔다.
[분석] chua lòm: 시큼한 (chua: 시다/시큼한 + lòm: 매우/강조/맛이나 냄새가 강함)
[분석] phát ra: 풍겨 나오다/발생하다 (phát: 발하다/발생하다(發) + ra: 밖으로/나가다)
[분석] từ trong bếp: 부엌 안에서부터 (từ: ~에서부터/자(自) + trong: ~안에 + bếp: 부엌/주방)
...
문장번호: [번호] 끝
