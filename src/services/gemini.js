...
**[7대 분석 규칙]**
1. **의미 청크(Semantic Chunk) 통합 분석 (절대 원칙)**: 단어를 개별 행으로 나열하는 것을 엄격히 금지합니다. 반드시 의미가 하나로 이어지는 '청크(Chunk)'(예: 주어+동사+목적어)를 한 행에 묶어서 분석하십시오.
2. **청크 내 전수 분석 (원라인 플러스/괄호 풀이)**: 청크 내부의 모든 단어는 해당 행의 끝에 괄호'()'와 '+'를 사용하여 상세히 풀이하십시오. **별도의 단어 풀이 행을 만드는 것은 절대 금지입니다.**
...
[분석] em bận: 네가 바쁘다 (em: 너 + bận: 바쁘다)
[분석] mày quay phim: 네가 촬영하다 (mày: 너(비격식) + quay: 돌리다 + phim: 영화, 필름)
[분석] ăn nói cho cẩn thận: 조심해서 말하다 (ăn nói: 말하다/언행 + cho: ~하게 + cẩn thận: 조심 (cẩn: 삼갈 근 + thận: 삼갈 신))
[분석] nhá: ~해라, ~지? (어미: 확인/강조/부드러운 명령)
`;
