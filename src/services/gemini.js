...
4. **계층적 한 줄 분석(Hierarchical One-Line)**: 청크 전체 뜻을 먼저 쓰고, 그 뒤에 구성 단어들의 개별 의미를 괄호'()'와 '+'를 사용하여 같은 줄에 포함하십시오.
5. **한자어 노이즈 제거 (순수 한글 뜻만)**: 한자 병기(예: 評價)를 절대 하지 마십시오. 오직 순수 한국어 뜻만 간결하게 제공하십시오.
6. **문법 패턴 통합 필축**: 'nếu...thì', 'càng...càng' 등 상관 접속사 패턴을 최우선적으로 묶어서 설명하며, 반드시 **굵게(Bold)** 표시하십시오.
7. **초단축 미니멀리즘 (핵심 키워드)**: 모든 설명에서 서술형(~을 나타냄, ~임)을 배제하십시오. 오직 핵심 뜻과 기능 키워드(예: 완료, 존칭, 미래)만 단어 위주로 짧게 표기하십시오. 

**[출력 형식]**
[번역] 번역된 한국어 문장
[분석] 베트남어 청크: 한국어 전체 뜻 (단어: 뜻 + 단어: 뜻)

**[구현 예시]**
문장: viết rõ ràng ra một tí ạ.
[번역] 조금 더 명확하게 써주세요.
[분석] viết rõ ràng ra: 명확하게 쓰다 (viết: 쓰다 + rõ ràng: 분명히 + ra: 완료)
[분석] một tí ạ: 조금요 (một tí: 조금 + ạ: 존칭/요)
...
문장번호: [번호] 끝
