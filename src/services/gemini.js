...
전달된 미디어의 음성 트랙을 분석하여 정확한 타임라인 대본을 생성하되, **반드시 아래 주어진 [작업 순서]를 엄격히 지켜서 출력하십시오.**

[작업 순서 강제 (Pre-listening Simulation)]
절대 바로 타임스탬프 전사를 시작하지 마십시오. 먼저 오디오의 전체 흐름을 "1. [사전 통독]"에 요약한 뒤, 그 요약본을 머리에 담고 비로소 "2. [본 전사]"를 시작하십시오.

=== 1. [사전 통독] ===
오디오를 끝까지 다 듣고 3~5줄로 짧게 요약 문단을 출력하십시오. 
이 문단에는 주요 등장인물 이름, 장소나 분위기, 핵심 상황과 줄거리가 유추된 단어와 함께 명시되어야 합니다.

=== 2. [본 전사] ===
본인이 위 1번에서 작성한 요약본 문맥을 기반으로, 0초부터 끝까지 들리는 실제 소리를 다음 규칙에 따라 한 줄씩 전사하십시오:
...
2. 오직 명확한 인간의 음성만! 화면에 보이는 자막/글씨, 배경음악, 빈칸 지어내기는 철저히 금지합니다.
3. 의미 단위 세밀 청킹: 논리적인 곳에서 끊어 5~10단어 이내로 쪼개십시오.
4. **유사 발음(Homophone) 및 문맥 호응(Collocation) 보정 강제**: 베트남어 성조 등 발음이 헷갈리는 구간(예: 'đưa hẳn hoi' vs 'đứa ăn hôi')에서는 절대 단순 음향 스펙트럼에만 의존하지 마십시오. 앞뒤 단어의 행동 논리(예: 'Nhặt lên(주워라)' 뒤에는 사물을 건네는 'đưa'가 논리적으로 맞음)를 최우선으로 분석하여, 위 1번의 [사전 통독] 내용과 100% 부합하는 가장 합리적이고 자연스러운 단어로 전사하십시오.
