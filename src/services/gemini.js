...
5. **한자 기반 풀이 통합**: 한자 기반 단어 분석 시에도 별도의 행을 생성하지 마십시오. 한글 뜻풀이(음과 뜻)를 단어 설명 행 안에 포함하십시오.
   - 예: tiền cọc: 보증금 (tiền (돈 전) + cọc (보증))
6. **핵심 패턴 볼드 강조**: 단순 복합어가 아닌, 문법적 구조나 회화 핵심 **패턴(Pattern)**에 해당하는 표현만 **볼드** 처리하십시오.
...
5. **한자 기반 풀이 통합**: 한자 뜻풀이(음과 뜻)를 별도 행 없이 단어 설명 행 안에 포함하십시오.
6. **핵심 패턴 볼드 강조**: 문법적 구조나 회화 핵심 **패턴(Pattern)**만 **볼드** 처리하십시오.
