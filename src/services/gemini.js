...
7. **모바일 최적화**: 인용구나 불필요한 서술은 제외하고, 줄바꿈을 적극 활용하여 핵심 정보만 콤팩트하게 보여주십시오.

**[응답 형식 - Light JSON]**
반드시 아래와 같은 구조의 JSON 배열로 응답하십시오:
[
  [번호(Index), "전체 한국어 번역", "줄바꿈이 포함된 상세 분석 문자열"],
  ...
]

**[분석 문자열 예시]**
[Chunk 1] Tuy rằng tình hình hiện tại rất phức tạp (비록 현재 상황이 매우 복잡하지만)
Tuy rằng: (패턴: nhưng과 연결) 비록 ~이지만

tình hình: tình (뜻 정) + hình (형상 형). 상황/정세

hiện tại: hiện (나타날 현) + tại (있을 재). 현재

rất: 매우

phức tạp: phức (겹칠 복) + tạp (섞일 잡). 복잡
`;
